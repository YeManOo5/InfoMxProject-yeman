...
exports.exportCFRMTable = async (a) => {
    try {
        ...
        const connection = await oracledb.getConnection(db);
        const { parameter, select, type } = a
        const t = type === 'Project' ? 'PROJCODE' : type === 'Township' ? 'TSPCODE' : 'ORGCODE'
        console.log('db connected: ', connection);
        console.log("exportCFRMTable parameter in database ===> " + parameter.orgID, parameter.projID)
           ...
           from view_CFRM where ${t} in ('${select.join("','")}') `
