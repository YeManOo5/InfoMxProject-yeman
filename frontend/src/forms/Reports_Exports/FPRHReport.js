...
import * as fprhRp from '../../modals/fprhreport'
...
export default function FPRHReport() {
  ...
  /////Report Data/////////////
  const indicator = [
    'Number of 10 year IUD (Copper-T) provided during reporting period',
    'Number of 5 year IUD (Multiload)provided during reporting period',
    'Number of 5 year implant (Jadelle)provided during reporting period',
    'Number of 4 year implant (Zarin or Sino-Implant)provided during reporting period',
    'Number of 3 year implant (Implanon)provided during reporting period',
    'Number of 3 month Injectable Depo provided during reporting period',
    'Number of Combined Oral Contraceptive Pill provided during reporting period (cycle)',
    'Number of Progestogen Only Pill provided during reporting period (Cycle)',
    'Number of Male condoms provided during reporting period',
    'Number of Female Condoms provided during reporting period',
    'Number of Emergency contraception provided during reporting period (Strips)',
    'Total number of clean delivery kit provided',
    'Total number of new born kit provided'
  ]
  const [fprhData, setfprhData] = useState([])
  ...
  const reportBtn = async () => {
    ...
    setLoading(true)
    var fprh = await fprhRp.fprhReport({ parameter, select, type:0});
    var fpList = []

    if (fprh) {
      fpList.push(fprh.data.data.fprhReportOne)
      fpList.push(fprh.data.data.fprhReportTwo)
      fpList.push(fprh.data.data.fprhReportThree)
      fpList.push(fprh.data.data.fprhReportFour)
      fpList.push(fprh.data.data.fprhReportFive)
      fpList.push(fprh.data.data.fprhReportSix)
      fpList.push(fprh.data.data.fprhReportSeven)
      fpList.push(fprh.data.data.fprhReportEight)
      fpList.push(fprh.data.data.fprhReportNine)
      fpList.push(fprh.data.data.fprhReportTen)
      fpList.push(fprh.data.data.fprhReportEleven)
      fpList.push(fprh.data.data.fprhReportTwelve)
      fpList.push(fprh.data.data.fprhReportThirteen)
      setfprhData(fpList)
      console.log("fprh  =>", fpList)
  ...
  const exportBtn = () => {
    ...
    var tbl = document.getElementById('fprhtable');
    ...
    XLSX.utils.book_append_sheet(wbb, wss, 'FP+RH_Report');
    XLSX.writeFile(wbb, "FP+RH_Report" + `_${moment(new Date()).format('DD-MM-YYYY')}` + '.xlsx');
  }
  ...
  return (
    <>
      ...
      <Typography variant="h5" align="center" style={{ color: '#53344d', background: '#fcf0f2', fontWeight: 'bold', padding: '1%' }}>
        Family Planning and Reproductive Heatlh Care Services Report </Typography>
      <Grid container
        direction="row"
      ...
      <div style={{ justifyContent: 'center', alignItems: 'center' }}>
        <Table id="fprhtable" size="small" className={classes.table}>
          <TableBody>
            <TableRow>
            ...
            {indicator.map((indi, index) => (
              <>
                <TableRow>
                  ...
                  {fprhData.length ? fprhData[index].map((data) => (
