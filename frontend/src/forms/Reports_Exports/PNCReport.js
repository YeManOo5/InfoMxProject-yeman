...
export default function PNCReport() {
  ...
  /////Report Data/////////////
  const indicator = [
    'Number of women who received at least one PNC within 2 days after delivery',
    'Number of women who received at least one PNC provided by skilled providers within 2 days after delivery',
    'Total number of women received at least one PNC within 42 days after delivery',
    'Total number of women received at least one PNC within 42 days after delivery provided by skilled providers',
    'Total number of PNC services provided by health workers within 42 days after delivery',
    'Total number of PNC services provided by Skilled providers within 42 days after delivery',
    'Total number of Health Education services provided by health workers during PNC services',
    'Total number of women who received vitamin B1 supplementation during PNC services',
    'Total number of women who received vitamin A supplementation during PNC services',
    'Total number of referral cases to higher facilities due to any complications within 42 days after delivery',
    'Total number of referral cases to Government Health facilities due to any complications within 42 days after delivery',
  ]
  const [PNCNewData, setPNCNewData] = useState([])
  const [PNCOldData, setPNCOldData] = useState([])
  ...
  const reportBtn = async () => {
    ...
    setLoading(true)
    var pncOld = await pncRp.pncReport({ parameter, select, type: 1 });
    var pncNew = await pncRp.pncReport({ parameter, select, type: 2 });
    var oldList = []
    var newList = []

    if (pncOld && pncNew) {
      oldList.push(pncOld.data.data.pncReportOne)
      oldList.push(pncOld.data.data.pncReportTwo)
      oldList.push(pncOld.data.data.pncReportThree)
      oldList.push(pncOld.data.data.pncReportFour)
      oldList.push(pncOld.data.data.pncReportFive)
      oldList.push(pncOld.data.data.pncReportSix)
      oldList.push(pncOld.data.data.pncReportSeven)
      oldList.push(pncOld.data.data.pncReportEight)
      oldList.push(pncOld.data.data.pncReportNine)
      oldList.push(pncOld.data.data.pncReportTen)
      oldList.push(pncOld.data.data.pncReportEleven)
      newList.push(pncNew.data.data.pncReportOne)
      newList.push(pncNew.data.data.pncReportTwo)
      newList.push(pncNew.data.data.pncReportThree)
      newList.push(pncNew.data.data.pncReportFour)
      newList.push(pncNew.data.data.pncReportFive)
      newList.push(pncNew.data.data.pncReportSix)
      newList.push(pncNew.data.data.pncReportSeven)
      newList.push(pncNew.data.data.pncReportEight)
      newList.push(pncNew.data.data.pncReportNine)
      newList.push(pncNew.data.data.pncReportTen)
      newList.push(pncNew.data.data.pncReportEleven)

      setPNCOldData(newList)
      setPNCNewData(oldList)
      console.log("PNC Old =>", PNCOldData)
      console.log("PNC New =>", PNCNewData)
    }
    setLoading(false)
  ...
  const exportBtn = () => {
    ...
    var tbl = document.getElementById('pnctable');
    ...
    XLSX.utils.book_append_sheet(wbb, wss, 'PNCReport');
    XLSX.writeFile(wbb, "PNCReport" + `_${moment(new Date()).format('DD-MM-YYYY')}` + '.xlsx');
  }
      ...
      {allICD ? <CLRReport /> :
        <>
          ...
          <Typography variant="h5" align="center" style={{ color: '#53344d', background: '#fcf0f2', fontWeight: 'bold', padding: '1%' }}>
            Post Natal Care Report </Typography>
          <Grid container
            direction="row"
            ...
            {(PNCNewData.length && PNCOldData.length) ?
              <Grid item xs={6} sm={4} md={'auto'}>
          ...
          <div style={{ justifyContent: 'center', alignItems: 'center' }}>
            <Table id="pnctable" size="small" className={classes.table}>
              <TableBody>
                <TableRow>
