...
export default function HeadCountReport() {
  ...
  const [HC1NewData, setHC1NewData] = useState([])
  const [HC1OldData, setHC1OldData] = useState([])
  ...
  const reportBtn = async () => {
    ...
    setLoading(true)
    var hcOld = await hcRp.hcReport({ parameter, select, type: 2 });
    var hcNew = await hcRp.hcReport({ parameter, select, type: 1 });
    var oldList = []
    var newList = []

    if (hcOld && hcNew) {
      oldList.push(hcOld.data.data.hcReportOne)
      oldList.push(hcOld.data.data.hcReportTwo)
      oldList.push(hcOld.data.data.hcReportThree)
      oldList.push(hcOld.data.data.hcReportFour)
      oldList.push(hcOld.data.data.hcReportFive)
      oldList.push(hcOld.data.data.hcReportSix)
      oldList.push(hcOld.data.data.hcReportSeven)
      oldList.push(hcOld.data.data.hcReportEight)
      newList.push(hcNew.data.data.hcReportOne)
      newList.push(hcNew.data.data.hcReportTwo)
      newList.push(hcNew.data.data.hcReportThree)
      newList.push(hcNew.data.data.hcReportFour)
      newList.push(hcNew.data.data.hcReportFive)
      newList.push(hcNew.data.data.hcReportSix)
      newList.push(hcNew.data.data.hcReportSeven)
      newList.push(hcNew.data.data.hcReportEight)

      setHC1OldData(oldList)
      setHC1NewData(newList)
      console.log("HC Old =>", HC1OldData)
      console.log("HC New =>", HC1NewData)
    }
    setLoading(false)
  ...
  const exportBtn = () => {
    ...
    var tbl = document.getElementById('hctable');
    ...
    XLSX.utils.book_append_sheet(wbb, wss, 'HeadCountReport');
    XLSX.writeFile(wbb, "HeadCountReport" + `_${moment(new Date()).format('DD-MM-YYYY')}` + '.xlsx');
  }
      ...
      {allICD ? <AllICDReport /> :
        <>
          ...
          <Grid container
            ...
            {(HC1NewData.length && HC1OldData.length) ?
              <Grid item xs={6} sm={4} md={'auto'}>
          ...
          <div style={{ justifyContent: 'center', alignItems: 'center' }}>
            <Table id="hctable" size="small" className={classes.table}>
              <TableBody>
                <TableRow>
