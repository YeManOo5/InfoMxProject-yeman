...
import * as fpRp from '../../modals/fpreport'
...
export default function FPReport() {
  ...
  /////Report Data/////////////
const indicator = [
  'Total Number of family planning client visits',
  'Total Number of family planning client visit for short- term method',
  'Total Number of family planning client visit for long- term method',
  'Total number of family planning client visit for depo injection',
  'Total number of family planning client visit for Combined Oral Contraceptive Pills',
  'Total number of family planning client visit for Progestogen Only Pills',
  'Total number of family planning client visit for Condoms',
  'Total number of family planning client visit for EC Pills',
  'Total Number of family planning client visit for Implant Insertion',
  'Total Number of family planning client visit for IUD Insertion',
  'Total Number of family planning client visit for Counseling Services',
  'Total Number of family planning client visit who received UCG Test ',
  'Total Number of family planning client visit who received HIV Testing and Counseling'
]
const [fpNewData, setfpNewData] = useState([])
const [fpOldData, setfpOldData] = useState([])
...
const reportBtn = async () => {
  ...
  setLoading(true)
  var fpOld = await fpRp.fpReport({ parameter, select, type: 2 });
  var fpNew = await fpRp.fpReport({ parameter, select, type: 1 });
  var oldList = []
  var newList = []

  if (fpOld && fpNew) {
    oldList.push(fpOld.data.data.fpReportOne)
    oldList.push(fpOld.data.data.fpReportTwo)
    oldList.push(fpOld.data.data.fpReportThree)
    oldList.push(fpOld.data.data.fpReportFour)
    oldList.push(fpOld.data.data.fpReportFive)
    oldList.push(fpOld.data.data.fpReportSix)
    oldList.push(fpOld.data.data.fpReportSeven)
    oldList.push(fpOld.data.data.fpReportEight)
    oldList.push(fpOld.data.data.fpReportNine)
    oldList.push(fpOld.data.data.fpReportTen)
    oldList.push(fpOld.data.data.fpReportEleven)
    oldList.push(fpOld.data.data.fpReportTwelve)
    oldList.push(fpOld.data.data.fpReportThirteen)

    newList.push(fpNew.data.data.fpReportOne)
    newList.push(fpNew.data.data.fpReportTwo)
    newList.push(fpNew.data.data.fpReportThree)
    newList.push(fpNew.data.data.fpReportFour)
    newList.push(fpNew.data.data.fpReportFive)
    newList.push(fpNew.data.data.fpReportSix)
    newList.push(fpNew.data.data.fpReportSeven)
    newList.push(fpNew.data.data.fpReportEight)
    newList.push(fpNew.data.data.fpReportNine)
    newList.push(fpNew.data.data.fpReportTen)
    newList.push(fpNew.data.data.fpReportEleven)
    newList.push(fpNew.data.data.fpReportEleven)
    newList.push(fpNew.data.data.fpReportTwelve)
    newList.push(fpNew.data.data.fpReportThirteen)

    setfpNewData(newList)
    setfpOldData(oldList)
    console.log("fp Old =>", fpOldData)
    console.log("fp New =>", fpNewData)
  }
  setLoading(false)
...
const exportBtn = () => {
  ...
  var tbl = document.getElementById('fptable');
  ...
  XLSX.utils.book_append_sheet(wbb, wss, 'FPReport');
  XLSX.writeFile(wbb, "FPReport" + `_${moment(new Date()).format('DD-MM-YYYY')}` + '.xlsx');
}
  ...
  return (
    ...
    <Modals open={loading} />
      <Typography variant="h5" align="center" style={{ color: '#53344d', background: '#fcf0f2', fontWeight: 'bold', padding: '1%' }}>
        Family Planning Report </Typography>
        <Grid container
        direction="row"
        ...
        {(fpNewData.length && fpOldData.length) ?
          <Grid item xs={6} sm={4} md={'auto'}>
        ...
        <Table id="fptable" size="small" className={classes.table}>
          <TableBody>
            <TableRow>
