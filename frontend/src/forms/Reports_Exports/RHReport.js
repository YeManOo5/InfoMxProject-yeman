...
import * as rhRp from '../../modals/rhreport'
...
export default function RHReport() {
  ...
  const [RHNewData, setRHNewData] = useState([])
  const [RHOldData, setRHOldData] = useState([])
  ...
  const reportBtn = async () => {
    ...
    setLoading(true)
    var rhOld = await rhRp.rhReport({ parameter, select, type: 2 });
    var rhNew = await rhRp.rhReport({ parameter, select, type: 1 });
    var oldList = []
    var newList = []

    if (rhOld && rhNew) {
      oldList.push(rhOld.data.data.rhReportOne)
      oldList.push(rhOld.data.data.rhReportTwo)
      oldList.push(rhOld.data.data.rhReportThree)
      oldList.push(rhOld.data.data.rhReportFour)
      oldList.push(rhOld.data.data.rhReportFive)
      oldList.push(rhOld.data.data.rhReportSix)

      newList.push(rhNew.data.data.rhReportOne)
      newList.push(rhNew.data.data.rhReportTwo)
      newList.push(rhNew.data.data.rhReportThree)
      newList.push(rhNew.data.data.rhReportFour)
      newList.push(rhNew.data.data.rhReportFive)
      newList.push(rhNew.data.data.rhReportSix)

      setRHOldData(oldList)
      setRHNewData(newList)
      console.log("RH Old =>", RHOldData)
      console.log("RH New =>", RHNewData)
    }
    setLoading(false)
  ...
  const exportBtn = () => {
    ...
    var tbl = document.getElementById('rhtable');
    ...
    XLSX.utils.book_append_sheet(wbb, wss, 'RHReport');
    XLSX.writeFile(wbb, "RHReport" + `_${moment(new Date()).format('DD-MM-YYYY')}` + '.xlsx');
  }
  ...
  return (
    <>
      ...
      <Typography variant="h5" align="center" style={{ color: '#53344d', background: '#fcf0f2', fontWeight: 'bold', padding: '1%' }}>
        Reproductive Health Care Report </Typography>
      <Grid container
        direction="row"
        ...
        {(RHNewData.length && RHOldData.length) ?
          <Grid item xs={6} sm={4} md={'auto'}>
      ...
      <div style={{ justifyContent: 'center', alignItems: 'center' }}>
        <Table id="rhtable" size="small" className={classes.table}>
          <TableBody>
            <TableRow>
