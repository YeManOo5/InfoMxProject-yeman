...
////////Controls////////
import CustomOTPDeleteDialog from "../../components/controls/CustomOTPDeleteDialog";
...
const EditOTPForm = props => {
    ...
    const editButtonHandle = (event) => {
        ...
        sessionStorage.setItem('editOTPPatient', true)
    ...
    const deleteButtonHandle = (event) => {
        console.log(event.currentTarget.value)
        sessionStorage.setItem('deleteOTPPatient', "true")
    ...
    return (
        <div style={{ background: '#fcf0f2' }}>
            {dialogOpen && <CustomOTPDeleteDialog onClose={setDialogCloseControl} open={setDialogOpenControl} />}
            <Modals open={loading} />
            <Card style={{ background: '#fcf0f2' }}>
                {(editPage === "false") ?
                    <><Typography variant="h5" align="center" style={{ color: '#53344d', background: '#fcf0f2', fontWeight: 'bold', padding: '1%' }}>
                        IMAM-OTP Information</Typography>
                        <TableContainer
