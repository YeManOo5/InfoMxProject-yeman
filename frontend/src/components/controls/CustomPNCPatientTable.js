...
import CustomPNCDeleteDiaglog from '../controls/CustomPNCDeleteDiaglog';
...
export default function CustomPNCPatientTable(props) {
    ...
    const editButtonHandle = (event) => {
        ...
        sessionStorage.setItem('editPNCPatient', true)
    ...
    const deleteButtonHandle = (event) => {
        console.log(event.currentTarget.value)
        sessionStorage.setItem('deletePNCPatient', "true")
    ...
    return (
        <>
            {dialogOpen && <CustomPNCDeleteDiaglog onClose={setDialogCloseControl} open={setDialogOpenControl} />}
            <TableContainer
