...
import CustomGMDeleteDiaglog from '../controls/CustomGMDeleteDiaglog';
...
export default function CustomGMPatientTable(props) {
    ...
    const editButtonHandle = (event) => {
        ...
        sessionStorage.setItem('editGMPatient', true)
    ...
    const deleteButtonHandle = (event) => {
        console.log(event.currentTarget.value)
        sessionStorage.setItem('deleteGMPatient', "true")
    ...
    return (
        <>
            {dialogOpen && <CustomGMDeleteDiaglog onClose={setDialogCloseControl} open={setDialogOpenControl} />}
            <TableContainer
