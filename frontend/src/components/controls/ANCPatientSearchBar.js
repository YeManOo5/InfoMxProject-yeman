...
import CustomANCPatientTable from './CustomANCPatientTable'
...
function PatientSearchBar({ placeholder, searchData, tableData }) {
  ...
  const handleFilter = (event) => {
    ...
    const newFilter = searchData.filter((value) => {
      return (value.ANREGID.toLowerCase()).includes(searchWord.toLowerCase());
    });
  ...
  return (
    ...
    <div>
        ...
        {filteredData.length != 0 && (
          <div className="dataResult">
            {filteredData.slice(0, 15).map((value, key) => {
              return (
                <List >
                  <ListItem button onClick={patientClickHandle} >{value.ANREGID} </ListItem>
                </List>
              );
            })}
          </div>
      ...
      <CustomANCPatientTable searchData={selectedPatient} tdata={tableData} /> 
    </div>
  );
}
