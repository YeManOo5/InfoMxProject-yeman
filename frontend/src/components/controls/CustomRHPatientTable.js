...
import CustomDeleteDialog from '../controls/CustomDeleteDialog';
...
export default function CustomRHPatientTable(props) {
    ...
    const editButtonHandle = (event) => {
        ...
        sessionStorage.setItem('editRHPatient', true)
    ...
    const deleteButtonHandle = (event) => {
        console.log(event.currentTarget.value)
        sessionStorage.setItem('deleteRHPatient', "true")
    ...
    return (
        <>
            {dialogOpen && <CustomDeleteDialog onClose={setDialogCloseControl} open={setDialogOpenControl} />}
            <TableContainer
