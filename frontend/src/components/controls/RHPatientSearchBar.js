...
import CustomRHPatientTable from './CustomRHPatientTable'
...
function PatientSearchBar({ placeholder, searchData, tableData }) {
  ...
  const handleFilter = (event) => {
    ...
    const newFilter = searchData.filter((value) => {
      return (value.RHREGID.toLowerCase()).includes(searchWord.toLowerCase());
    });
  ...
  return (
    ...
    <div>
        ...
        {filteredData.length != 0 && (
          <div className="dataResult">
            {filteredData.slice(0, 15).map((value, key) => {
              return (
                <List >
                  <ListItem button onClick={patientClickHandle} >{value.RHREGID} </ListItem>
                </List>
              );
            })}
          </div>
      ...
      <CustomRHPatientTable searchData={selectedPatient} tdata={tableData} /> 
    </div>
  );
}
