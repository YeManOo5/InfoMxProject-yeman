...
import CustomIPDDeleteDialog from './CustomIPDDeleteDialog';
...
export default function CustomIPDPatientTable(props) {
    ...
    const editButtonHandle = (event) => {
        ...
        sessionStorage.setItem('editIPDPatient', true)
    ...
    const deleteButtonHandle = (event) => {
        console.log(event.currentTarget.value)
        sessionStorage.setItem('deleteIPDPatient', "true")
    ...
    return (
        <>
            {dialogOpen && <CustomIPDDeleteDialog onClose={setDialogCloseControl} open={setDialogOpenControl} />}
            <TableContainer
