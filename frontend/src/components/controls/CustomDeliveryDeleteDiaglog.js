...
//////////////////API//////////////////
import * as deli from '../../modals/deliinfo'
...
export default function CustomDeleteDialog(props) {
    ...
    const deleteHandle = async() => {
      ...
      DELIREGID : sessionStorage.getItem('deleteServicePatientID').split(',')[0],
      DELIUPDATE : moment(new Date()).format('YYYY-MM-DD hh:mm:ss')
     }
     ...
     const labData = {  ID : sessionStorage.getItem('deleteServicePatientID').split(',')[1],
      ...
      LABUPDATE : moment(new Date()).format('YYYY-MM-DD hh:mm:ss')
        ...
        const rhres = await deli.deleteDELI(rhData);
