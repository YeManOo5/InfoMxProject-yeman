...
//////////////////API//////////////////
import * as ipd from '../../modals/ipdinfo'
...
export default function CustomIPDDeleteDialog(props) {
    ...
    const deleteHandle = async() => {
      ...
      IPDREGID : sessionStorage.getItem('deleteServicePatientID').split(',')[0],
      IPDUPDATE : moment(new Date()).format('YYYY-MM-DD hh:mm:ss')
     }
     ...
     const labData = {  ID : sessionStorage.getItem('deleteServicePatientID').split(',')[1],
      ...
      LABUPDATE : moment(new Date()).format('YYYY-MM-DD hh:mm:ss')
        ...
        const rhres = await ipd.deleteIPD(rhData);
